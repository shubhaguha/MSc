...
class TextIterator:
    ...
    def __init__(self, source, target,
        ...
        if shuffle_each_epoch:
            ...
            if edit_vectors:
                self.edits_orig = edit_vectors
                self.source, self.target, self.edits = shuffle.main([self.source_orig, self.target_orig, self.edits_orig], temporary=True)
            else:
                self.edits_orig = None
                self.edits = None
            ...
            if edit_vectors:
                self.edits = fopen(edit_vectors, 'r')
            else:
                self.edits = None
        self.source_dicts = []
    ...
    def next(self):
        ...
        if len(self.source_buffer) == 0:
            for ss in self.source:
                ...
                if self.edits:
                    ee = map(int, self.edits.readline().split())
            ...
            if self.sort_by_length:
                ...
                if self.edits:
                    _ebuf = [self.edits_buffer[i] for i in tidx]

                self.source_buffer = _sbuf
                self.target_buffer = _tbuf
                if self.edits:
                    self.edits_buffer = _ebuf

            else:
                self.source_buffer.reverse()
                self.target_buffer.reverse()
                if self.edits:
                    self.edits_buffer.reverse()
